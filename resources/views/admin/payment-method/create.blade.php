...
@section('content')
    <section class="bg-primary-800 flex justify-center items-center rounded-2xl py-24">
        <div class="w-2/5 flex flex-col gap-4">
            <h1 class="text-text-950 bg-secondary-400 p-2 rounded-lg text-center">Tambah Metode Pembayaran</h1>
            @if ($errors->any())
            ...
            @endif
            <div class="px-3">
                <form action="{{ route('payment-method.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-3">
                    @csrf
                    @method('POST')
                    <x-form-group 
                        inputTag="input" 
                    ...
                    />
                    <button type="submit" class="bg-primary-500 hover:bg-primary-400 hover:text-text-950 hover:scale-[1.01] transition-all duration-500 ease rounded-lg mt-3 p-2 w-full">Tambah Metode Pembayaran</button>
                </form>
            </div>
        </div>
    </section>
